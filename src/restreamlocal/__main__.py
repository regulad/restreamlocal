...
def main() -> None:
    ...
    def overwrite_clipboard(*args, **kwargs) -> None:
    ...
    # Description
    description = tk.Label(window, text="ReStreamLocal provides an easy-to-use locally hosted alternative to restream.io.")
    description.pack()
    ...
    def get_stream_url() -> str:
        return f"rtmp://{local_host_entry.get()}:{local_port_entry.get()}/live"
    ...
    copy_url_button.bind("<Button-1>", lambda _: overwrite_clipboard(get_stream_url()))
    copy_url_button.pack(side=tk.LEFT)
    copy_key_button = tk.Button(mona_button_frame, text="Copy Stream Key")
    copy_key_button.bind("<Button-1>", lambda _: overwrite_clipboard(stream_key_entry.get()))
    ...
    def get_mona_server_directory() -> Path:
        ...
        temp_base_directory = Path(tempfile.gettempdir())
        mona_server_tempdir = temp_base_directory / "restreamlocal_mona_server"
        mona_server_executable = mona_server_tempdir / "MonaServer.exe"
        if mona_server_tempdir.exists() and mona_server_executable.exists():
            print("Using existing MonaServer")
            print(mona_server_tempdir)
            return mona_server_tempdir

        # if we didn't, make one
        mona_server_zip = get_mona_server_zip()
        mona_server_tempdir.mkdir(exist_ok=True)

        # all of the files of the zip are top level
        if mona_server_zip.is_file():
            with zipfile.ZipFile(cast(Path, mona_server_zip), "r") as zip_ref:
                zip_ref.extractall(mona_server_tempdir)
        else:
    ...
    number_of_remote_hosts = 1  # volatile variable to keep track of the number of remote hosts
    remote_host_tuple_getters: list[Callable[[], tuple[str, str]]] = []  # list of functions that return a remote url, stream key, and any aditional info

    # +/- buttons
    remote_host_button_frame = tk.Frame(remote_host_header_frame)

    def handle_add_host(event: tk.Event[tk.Button]) -> None:
    ...
    def handle_remove_host(event: tk.Event[tk.Button]) -> None:
        nonlocal number_of_remote_hosts
    ...
    def render_remote_host_entry() -> None:
        ...
        for i in range(number_of_remote_hosts):
            ...
            remote_host_label.pack(side=tk.LEFT)
            remote_host_entry = tk.Entry(remote_host_url_frame)
            ...
            remote_host_key_entry = tk.Entry(remote_host_key_frame)
    ...
    def get_ffmpeg_executable() -> Path:
        ...
        temp_base_directory = Path(tempfile.gettempdir())
        ffmpeg_tempdir = temp_base_directory / "restreamlocal_ffmpeg"
        ffmpeg_executable = ffmpeg_tempdir / "ffmpeg.exe"
        if ffmpeg_tempdir.exists() and ffmpeg_executable.exists():
            print("Using existing ffmpeg")
            print(ffmpeg_tempdir)
            return ffmpeg_executable

        # if we didn't, make one
        ffmpeg_tempdir.mkdir(exist_ok=True)
    ...
    def start_ffmpeg_process() -> None:
        ...
        remote_host_urls = []
        for remote_host_tuple_getter in remote_host_tuple_getters:
        ...
        current_ffmpeg_subprocess = subprocess.Popen(
            [
                ...
                "-i",
                f"{get_stream_url()}/{stream_key_entry.get()}",
                "-c:v",
                "copy", # no reencoding
                "-c:a",
                "copy",
